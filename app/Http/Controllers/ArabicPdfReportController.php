...
class ArabicPdfReportController extends Controller
{
    /**
     * Generate an Arabic PDF report for an investor's startup matches
     * 
    ...
    {
        try {
            Log::info('Arabic PDF Download Initiated', [
            ...
            // Verify view exists
            if (!View::exists('investors.pdf.ai_response_ar')) {
                Log::error("Arabic PDF view not found");
                return back()->with('error', 'قالب PDF غير موجود');
            }

            // Generate PDF filename
            $filename = sprintf(
                'investor_matches_ar_%s_%s.pdf',
                str_replace(' ', '_', $investor->name),
                now()->format('Y-m-d_H-i-s')
            );

            // Generate PDF using Snappy
            $pdf = SnappyPdf::loadView('investors.pdf.ai_response_ar', [
                'investor' => $investor,
                'startups' => $startups,
            ...
            Log::info('Arabic PDF Generated Successfully', [
                'filename' => $filename
            ]);
            ...
            // Log detailed error
            Log::error('Arabic PDF Generation Error', [
                'message' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);

            // Return user-friendly error
            return back()->with('error', 'تعذر إنشاء تقرير PDF. يرجى المحاولة مرة أخرى لاحقًا');
        }
    }
